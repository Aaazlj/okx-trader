"""
SMMA 支撑做多策略
从 okx-smma-trader/smma_strategy_long.py 移植
"""
import numpy as np
import pandas as pd
...
class SMMALongStrategy(IStrategy):
    """
    SMMA 支撑 + 放量阳线短线做多策略

    开仓条件（与做空镜像对称）：
    1. SMMA 趋势向上: current_smma >= prev_smma
    2. 价格在 SMMA 上方 % 范围内
    3. 当前K线为阳线（close > open）
    4. 阳线实体占比 >= bodyPercent
    ...
    def check_signal(self, df: pd.DataFrame, params: dict) -> dict | None:
        ...
        is_trending_up = current_smma >= prev_smma
        ...
        candle_range = highs[idx] - current_low
        body_size = abs(current_close - current_open)
        is_solid_body = candle_range > 0 and (body_size / candle_range) * 100 >= body_percent

        distance_pct = ((current_close - current_smma) / current_smma) * 100
        is_within_range = (current_close > current_smma) and (distance_pct <= pct_threshold)
        is_bullish = current_close > current_open

        if not (is_bullish and is_high_vol and is_solid_body and is_within_range
                and is_trending_up and ema_bias_ok and htf_trend_ok):
            return None

        stop_price = current_low - stop_offset
        ...
        return {
            ...
            "reason": (
                f"🟢 做多 | SMMA({period})={current_smma:.2f} | "
    ...
    def compute_indicators(self, df: pd.DataFrame, params: dict) -> dict:
        ...
        dist_pct = ((price - smma_val) / smma_val * 100) if smma_val > 0 else 0
