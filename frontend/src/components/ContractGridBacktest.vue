...
import {
  ...
  getContractGridBacktestRecord,
  getContractGridBacktestRecords,
  runContractGridBacktest,
} from '../api'
...
async function runBacktest() {
  loading.value = true
  try {
    const { data } = await runContractGridBacktest({
      ...
      fee_rate: props.strategy.params?.fee_rate ?? 0.0005,
      slippage_pct: props.strategy.params?.slippage_pct ?? 0.02,
    })
    result.value = data
    await nextTick()
    renderChart()
...
async function loadRecords() {
  ...
  try {
    const { data } = await getContractGridBacktestRecords({
      symbol: form.value.symbol,
      limit: 20,
    })
...
async function loadRecordDetail(id: number) {
  recordsLoading.value = true
  try {
    const { data } = await getContractGridBacktestRecord(id)
    ...
    series: [
      {
        ...
        smooth: true,
        symbol: 'none',
        lineStyle: { width: 2, color: '#4D8C8C' },
      },
    ],
  }
...
<template>
  <div class="section contract-grid-backtest">
    <h3>合约网格回测</h3>
    ...
    <div class="grid-param-strip">
      <span>{{ gridModeLabel(strategy.params?.grid_mode || 'neutral') }}</span>
      <span>{{ strategy.params?.lower_price || '-' }} - {{ strategy.params?.upper_price || '-' }}</span>
      <span>{{ strategy.params?.grid_count || 0 }} 格</span>
      <span>{{ strategy.params?.total_margin_usdt || 0 }} USDT</span>
    </div>
    ...
    <div v-if="result?.summary" class="summary-grid">
      ...
      <div class="summary-item">
        <span>最大持仓格</span>
        <b>{{ result.summary.max_open_legs }}</b>
      </div>
      <div class="summary-item">
        <span>K线数量</span>
    ...
    <div v-if="result?.summary?.stopped_reason" class="empty-tip">{{ result.summary.stopped_reason }}</div>
    ...
    >
      ...
      <el-table-column label="方向" width="80">
        <template #default="{ row }">{{ row.direction === 'short' ? '空' : '多' }}</template>
      </el-table-column>
      <el-table-column prop="level_index" label="格位" width="70" />
      <el-table-column prop="entry_price" label="开仓价" width="110" />
      <el-table-column prop="exit_price" label="平仓价" width="110" />
...
<style scoped>
.contract-grid-backtest {
  padding: 20px;
}
.contract-grid-backtest h3 {
